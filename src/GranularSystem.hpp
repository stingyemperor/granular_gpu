...
private:
    ...
    // TODO add solver
    DArray<int> cell_start_fluid;
    DArray<int> cell_start_boundary;
    const float3 _space_size;
    const float _dt;
    const int3 _cell_size;
    DArray<int> buffer_int;
    
    void compute_boundary_mass();
	void neighbor_search(const std::shared_ptr<GranularParticles>& particles, DArray<int>& cell_start);
};